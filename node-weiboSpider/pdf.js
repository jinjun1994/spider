...
const baseUrl = "weiboSpider/weibo";
const pdfPath = baseUrl + "/pdfs";
...
(async () => {
  ...
  for (const item of list) {
    if (fs.statSync("weiboSpider/weibo/" + item).isDirectory()) {
      const json = fs
        .readdirSync("weiboSpider/weibo/" + item)
        .filter((e) => e.includes(".json"))[0];
      // json改动时间晚于pdf时间才更新
      if (json) {
        const jsonPath = "weiboSpider/weibo/" + item + "/" + json;
        const lastModified = fs.statSync(jsonPath).mtime;
        console.log(lastModified, "lastModified");

        const pdfFile = `weiboSpider/weibo/pdfs/${item}_微博合集_时间排序.pdf`;
        if (!fs.existsSync(pdfFile)) {
          path.push(jsonPath);
        } else {
          console.log("pdf stas", fs.statSync(pdfFile));
        }
      }
      console.log(json);
    }
...
async function makePdf(obj, { sort, image } = {}) {
  ...
  if (sort === "时间") {
    weibo.sort(
      (a, b) =>
        dayjs(b.publish_time).valueOf() - dayjs(a.publish_time).valueOf()
    );
  }
  if (sort === "热度") {
    weibo.sort(
      (a, b) =>
        ...
        b.comment_num +
        b.retweet_num -
        (a.up_num + a.comment_num + a.retweet_num)
    );
  }
  ...
  const pdf = fs.createWriteStream(
    `weiboSpider/weibo/pdfs/${nickname}_微博合集_${sort}排序${
      image ? "_有图" : ""
    }.pdf`
  );
  doc.pipe(pdf);
  ...
  doc.fillColor("blue").text(`${nickname}`, doc.x, doc.y, {
    lineBreak: false,
  ...
  .moveUp()
  .fillColor("black")
  .text(`  微博总数: `, doc.x+doc.widthOfString(nickname), doc.y, { lineBreak: false })
  .fillColor("blue")
  .text(weibo.length, doc.x, doc.y, );
  ...
  for (const item of weibo) {
    ...
    // doc.fillColor('blue');
    const link = `https://www.weibo.com/${id}/${weiboId}`;
    await addQrcodeImage(link, doc);
    doc.text(`原文地址: ${link}`, doc.x, doc.y, {
      link,
      width: doc.widthOfString(`原文地址: ${link}`),
    });
...
async function addQrcodeImage(url, doc) {
  try {
    const image = qr.imageSync(url);
    doc.image(
      image,
      doc.page.width - 72 - 50,
      doc.y - doc.heightOfString("2020") * 2,
      { width: 50, scale: 0.25, lineBreak: false }
    );
  } catch (error) {
    console.log(error);
  }
